...
{
    internal sealed class MockObjectRepository : InterLinq.Objects.IObjectSource
    {
        private readonly static string data = Properties.Resources.Customers;

        private List<string> digits = new List<string> { "zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine" };
        ...
        public IEnumerable<T> GetObjects<T>()
        {
            ...
            {
                return this.GetProductList().AsEnumerable() as IEnumerable<T>;
            }
            else if (typeof(Supplier).IsAssignableFrom(typeof(T)))
            {
                return this.GetSupplierList().AsEnumerable() as IEnumerable<T>;
            }
            else if (typeof(Customer).IsAssignableFrom(typeof(T)))
            {
                return this.GetCustomerList().AsEnumerable() as IEnumerable<T>;
            } if (typeof(string).IsAssignableFrom(typeof(T)))
            {
                return this.digits as IEnumerable<T>;
            }
            throw new ArgumentException("Generic Parameter is invalid.", "{T}");
        }

        public List<string> GetListOfDigits()
        {
            return this.digits;
        }
        ...
        private void CreateLists()
        {
            ...
            supplierList = new List<Supplier>(){
                    ...
                    new Supplier {SupplierName = "Bigfoot Breweries", Address = "3400_8th Avenue Suite 210", City = "Bend", Country = "USA"},
            ...
            using (System.Xml.XmlReader customerData = System.Xml.XmlReader.Create(new StringReader(data)))
            {
                customerList = (
                    from e in XDocument.Load(customerData).
        ...
        public IEnumerable<T> GetObjects<T>(string name, params object[] parameters)
        {
            IEnumerable<T> returnEnum = null;
            if (string.IsNullOrEmpty(name))
            {
                returnEnum = this.GetObjects<T>();
            }
            else
            {
                switch (name)
                {
                    case "CustomersByCity":
                        returnEnum = CustomersByCity(parameters[0].ToString()) as IEnumerable<T>;
                        break;
                    default:
                        returnEnum = this.GetObjects<T>();
                        break;
        ...
        public IEnumerable<Customer> CustomersByCity(string city)
        {
            return this.GetCustomerList().Where(w => w.City == city).ToList();
        }
    }
}
